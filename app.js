...
async function trackVisit() {
    ...
    // If tracked in last 30 minutes, skip
    if (lastVisitTime && (now - parseInt(lastVisitTime)) < 30 * 60 * 1000) {
    ...
    try {
        // Check if this IP visited in last 30 minutes
        const visitsRef = ref(database, 'visits');
        const snapshot = await get(visitsRef);
        
        if (snapshot.exists()) {
            const allVisits = Object.values(snapshot.val());
            const recentVisit = allVisits.find(visit => 
                visit.ip === visitorInfo.ip && 
                (now - visit.timestamp) < 30 * 60 * 1000 // 30 minutes
            );
            
            if (recentVisit) {
                console.log('IP visited recently, skipping duplicate');
